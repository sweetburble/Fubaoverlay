...
class Train:
    ...
    def train(self):
        ...
        loader_train = torch.utils.data.DataLoader(dataset_train, batch_size=batch_size, shuffle=True, num_workers=8, pin_memory=False, collate_fn=custom_collate_fn)
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
            ...
            for i, data in enumerate(loader_train, 1):
                ...
                assert input_a.size() == input_b.size(), f"Size mismatch: dataA {input_a.size()}, dataB {input_b.size()}"
    ...
    def test(self):
        ...
        loader_test = torch.utils.data.DataLoader(dataset_test, batch_size=batch_size, shuffle=False, num_workers=8, pin_memory=False, collate_fn=custom_collate_fn)
...
def custom_collate_fn(batch):
    dataA = [item['dataA'] for item in batch]
    dataB = [item['dataB'] for item in batch]
    # 모든 텐서의 크기를 동일하게 맞추기 위해 패딩 또는 크기 조정을 수행할 수 있습니다.
    dataA = torch.stack(dataA)
    dataB = torch.stack(dataB)
